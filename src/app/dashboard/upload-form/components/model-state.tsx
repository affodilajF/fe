...
import { Check, Cpu, Info, Calendar, Clock, Activity, Sparkles, Zap, Layers, Eye } from "lucide-react";
...
interface ModelStateProps {
    ...
    setRunningModel?: React.Dispatch<React.SetStateAction<JobDetectionResult | null>>;
...
const statusTheme = {
    Running: {
        ...
        progressBar: "bg-gradient-to-r from-orange-200 to-orange-500 shadow-orange-200",
    },
    Finished: {
        ...
        progressBar: "bg-gradient-to-r from-indigo-300 to-indigo-500 shadow-indigo-100",
    }
};
...
export default function ModelState({
    ...
    useEffect(() => {
        if (!runningModel || runningModel.job_status === "Done" || !setRunningModel) return;
        ...
        fetchEventSource(`${BASE_URL}/inference-status/${runningModel.job_id}`, {
            ...
            async onmessage(event) {
                try {
                    ...
                    const isDone = data.status?.toLowerCase() === "done" || data.event === "done";

                    if (isDone) {
                        setRunningModel((prev) => (prev ? { ...prev, job_status: "Done" } : null));
                        setCurrentFrame(0);
                        ctrl.abort();
                    }
                } catch (err) {
            ...
            <div className="flex flex-col gap-6 mt-6 animate-pulse">
                ...
                <div className="border-3 border-slate-100 rounded-[1.5rem] p-8 md:p-10 bg-slate-50/50 flex flex-col gap-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                        {[1, 2, 3, 4].map((i) => (
                            <div key={i} className="h-28 bg-white rounded-[2rem] border border-slate-100" />
                        ))}
                    </div>
                </div>
            </div>
    ...
    if (model === null) {
        return (
            <div
                className="my-6 bg-slate-50 border-3 border-slate-200 rounded-[1.5rem] p-12 flex flex-col items-center justify-center transition-all text-center relative overflow-hidden group"
            >
                ...
                <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto relative z-10">
                    <AddVideoDialog onSuccess={onUploadSuccess} />
                    <Settings />
                </div>
            </div>
        );
    ...
    const displayedCurrentFrames = !isRunning ? displayedTotalFrames : currentFrame;

    // Hitung progress: Jika Done selalu 100%, jika Running hitung rasionya
    const progress = !isRunning
        ? 100
        : (displayedTotalFrames > 0
            ? Math.min(Math.round((displayedCurrentFrames / displayedTotalFrames) * 100), 100)
            : 0);

    const handleCancelClick = () => {
        showConfirm({
            title: "Konfirmasi Pembatalan",
            message: "Hasil deteksi tidak akan disimpan ke database jika Anda melanjutkan.",
                ...
                >
                    {isRunning ? (
                        <div className="relative flex items-center justify-center">
                            ...
                            <span className={`absolute w-full h-full rounded-full animate-ping opacity-20 ${theme.ping}`}></span>
                        </div>
                    ) : (
                        <div className={`bg-indigo-600 w-6 h-6 flex items-center justify-center rounded-lg shrink-0 shadow-lg ${theme.bannerIcon}`}>
            ...
            >
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                    <div className="space-y-3">
                        <div className="flex items-center gap-3">
                            <div className={`p-2.5 rounded-xl shadow-inner ${theme.iconBox}`}>
                                {isRunning ? <Zap className="w-5 h-5 animate-pulse" /> : <Activity className="w-5 h-5" />}
                            </div>
                            <span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-slate-400">AI Detection Analysis</span>
                        </div>
                    ...
                    <div className="flex items-center group">
                        <div className={`flex items-center gap-4 backdrop-blur-xl px-6 py-3 rounded-[1.25rem] text-sm font-semibold border transition-all ${theme.badge}`}>
                            {isRunning ? (
                                <IconLoader className={`w-5 h-5 animate-spin ${theme.badgeIcon}`} />
                            ) : (
                ...
                <div className="space-y-2">
                    <div className="flex justify-between items-end px-2">
                        <div className="flex flex-col gap-1">
                            ...
                            <span className="text-sm font-bold text-slate-700">
                                {displayedCurrentFrames.toLocaleString()} <span className="text-slate-400 font-medium">/ {displayedTotalFrames.toLocaleString()} frames</span>
                            </span>
                        </div>
                    ...
                    <div className="w-full rounded-xl bg-gray-100 p-1.5"> {/* wrapper abu2 dan rounded */}
                        <div className="h-4 w-full rounded-lg overflow-hidden"> {/* container lekukan + border abu2 */}
                            <motion.div
                                initial={{ width: 0 }}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    <div className="bg-white/60 backdrop-blur-sm p-6 rounded-[1.5rem] border border-slate-200 flex flex-col gap-1 transition-all duration-500 hover:shadow-2xl hover:shadow-slate-200/50 hover:-translate-y-1">
                        <div className="flex items-center gap-2 text-slate-400">
                            ...
                            <span className="text-[9px] font-semibold uppercase tracking-wider">Job ID</span>
                        </div>
                    ...
                    <div className="bg-white/60 backdrop-blur-sm p-6 rounded-[1.5rem] border border-slate-200 flex flex-col gap-3 transition-all duration-500 hover:shadow-2xl hover:shadow-slate-200/50 hover:-translate-y-1">
                        <div className="flex items-center gap-2 text-slate-400">
                            ...
                            <span className="text-[11px] font-semibold uppercase tracking-wider">Video Date</span>
                        </div>
                        <span className="font-bold text-slate-800 text-xl">{getVideoDate(model.video_datetime)}</span>
                    </div>
                    ...
                    <div className="bg-white/60 backdrop-blur-sm p-6 rounded-[1.5rem] border border-slate-200 flex flex-col gap-3 transition-all duration-500 hover:shadow-2xl hover:shadow-slate-200/50 hover:-translate-y-1">
                        <div className="flex items-center gap-2 text-slate-400">
                            ...
                            <span className="text-[11px] font-semibold uppercase tracking-wider">Video Time</span>
                        </div>
                        <span className="font-bold text-slate-800 text-xl">{getVideoTime(model.video_datetime)}</span>
                    </div>
                    ...
                    <div className="bg-white/60 backdrop-blur-sm p-6 rounded-[1.5rem] border border-slate-200 flex flex-col gap-3 transition-all duration-500 hover:shadow-2xl hover:shadow-slate-200/50 hover:-translate-y-1">
                        <div className="flex items-center gap-2 text-slate-400">
                            <Activity className="w-4.5 h-4.5" />
                            <span className="text-[9px] font-semibold uppercase tracking-wider">Created at</span>
                        </div>
                        <span className="font-mono text-slate-500 text-xs font-bold break-words">{formatVideoDateTime(model.created_at)}</span>
                    </div>
                </div>
